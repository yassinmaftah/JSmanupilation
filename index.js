...
const bookings = spaceData.bookings;
const users = spaceData.users;
const packages = spaceData.packages;
const extras = spaceData.extras;
const promotions = spaceData.promotions;
...
function getAvailableDestinations() {
    ...
    {
        if (destination[i].available == true)
            count++;
        i++;
    }
    return count;
...
function findUserByEmail(email) {
    ...
    {
        const emailuser = users[i].email;
        if (users[i].email == email)
            return users[i];
        i++;
    }
}
// findUserByEmail(users[3].email);
...
function countTotalPassengers() {
    ...
    while (i < bookings.length)
    {
        totalpassangers += bookings[i].passengers.length;
        i++;
    }
...
function groupBookingsByStatus() {
    ...
    while (i < bookings.length)
    {
        if (bookings[i].status == "confirmed")
            confirm.push(bookings[i]);
        if (bookings[i].status == "pending")
            panding.push(bookings[i]);
        if (bookings[i].status == "cancelled")
            cancel.push(bookings[i]); 
        i++;
    } 
